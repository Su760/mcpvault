...
fn keygen_creates_file_with_valid_json_hex_keys() {
    ...
    let priv_hex = key["private_key"]
        .as_str()
        .expect("private_key field missing");
    let pub_hex = key["public_key"]
        .as_str()
        .expect("public_key field missing");
    assert_eq!(
        priv_hex.len(),
        64,
    ...
    assert_eq!(
        pub_hex.len(),
        64,
        "public_key should be 64 hex chars (32 bytes)"
    );
}
...
fn mint_verify_roundtrip_succeeds() {
    ...
    let token_b64 = String::from_utf8(mint_out.stdout)
        .unwrap()
        .trim()
        .to_string();
    ...
    let verify_out = mcpvault()
        .args([
            "verify", "--token", &token_b64, "--pubkey", pub_hex, "--tool", "db_query",
        ])
        .output()
        .unwrap();
    ...
    assert!(
        ...
        "verify should succeed for valid token"
    );
    let facts: serde_json::Value =
        serde_json::from_str(&String::from_utf8(verify_out.stdout).unwrap())
...
fn mint_attenuate_verify_roundtrip_succeeds() {
    ...
    let att_token = String::from_utf8(att_out.stdout)
        .unwrap()
        .trim()
        .to_string();
    assert!(
        !att_token.is_empty(),
        "attenuated token should be non-empty"
    );

    // verify attenuated token for db_query — should succeed
    let verify_out = mcpvault()
        .args([
            "verify", "--token", &att_token, "--pubkey", pub_hex, "--tool", "db_query",
        ])
        .output()
        .unwrap();
    assert!(
        verify_out.status.success(),
        "verify of attenuated token for allowed tool should succeed"
    );
}
...
fn inspect_shows_block_count_1_for_fresh_token() {
    ...
    let info: serde_json::Value =
        serde_json::from_str(&String::from_utf8(inspect_out.stdout).unwrap())
            .expect("inspect output should be valid JSON");
    assert_eq!(
        info["block_count"], 1,
        "fresh token should have block_count = 1"
    );
}
